...
import numpy as np
from util import warp_tqdm, get_metric, AverageMeter,prototypical_loss
from sklearn import manifold, datasets
from specaug import *
import h5py
# setting
print_freq = 1000
meta_val_way = 5  #can be change to solve bug
meta_val_shot = 5
...
class Trainer:
    ...
    def do_epoch(self, epoch, scheduler, disable_tqdm, model,
        ...
        for i, (input, target) in enumerate(tqdm_train_loader):
            # input=input.numpy().reshape(input.shape[0],17,128,1)
            # augmentation = SpecAugment(input,policy='SM',zero_mean_normalized=True).time_mask()
            # augmentation = SpecAugment(input,policy='SM',zero_mean_normalized=True).freq_mask()
            # input = torch.from_numpy(augmentation).view(augmentation.shape[0],17,128)
    ...
    def save_plot_data(self,feature_x,target_y,model):
        ...
        hf = h5py.File('/data/nouman/EVAL/checkpoint/plot/visual_transductive.h5', 'w')
